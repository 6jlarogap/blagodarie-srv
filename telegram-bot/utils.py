...
class Misc(object):
    ...
    # Это для диалога ввода местоположения.
    # Здесь в слове отмена костыль: O, e, a - латинские
    # Без костыля: юзер вместо нажатия на конопку Отмена набирает руками
    # Отмена, диалог отменяется, а кнопки задать местоположение и Отмена
    # висят! Сообщение с ReplyKeyboardRemove() не отрабатывает, если
    # руками ввел. А если на кнопку нажал, то отрабатывает!
    # Костыль, чтоб юзер сдуру не набрал руками Отмена.
    #
    PROMPT_CANCEL_LOCATION = 'Oтмeнa'
